    ...
    async main(@Body() payload: CreateSnapshotDto)
    {
        ...
        const messagesDetail = payload.messagesDetail.map(message => {
            return {
                ...
                flowHash: Utils.sha1(tenant.code + system.name + (message.flowParty ? message.flowParty : '') + message.flowComponent +  message.flowInterfaceName + message.flowInterfaceNamespace),
        ...
        const channelsDetail = payload.channelsDetail.map(channel => {
            return {
                ...
                channelHash: Utils.sha1(tenant.code + system.name + (channel.channelParty ? channel.channelParty : '') + channel.channelComponent + channel.channelName),

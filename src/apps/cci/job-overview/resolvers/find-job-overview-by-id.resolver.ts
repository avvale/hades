...
export class FindJobOverviewByIdResolver
{
    ...
    @TenantConstraint()
    async main(@CurrentAccount() account: AccountResponse, @Args('id') id: string, @Args('constraint') constraint?: QueryStatement, ): Promise<CciJobOverview>
    {
        return await this.queryBus.ask(new FindJobOverviewByIdQuery(id, constraint));
    }
}